...
namespace CSFramework {
	...
	public class SymbolManager : MonoBehaviour {
		...
		/// </summary>
		public Symbol GetRandomSymbol(int col) {
			var weights = GetWeights(col);
			if (weights.Length > 0) {
		...
		public float[] GetWeights(int col) {
			var weights = new float[symbols.Length];
			for (int i = 0; i < weights.Length; i++) {
				weights[i] = (i == 0 ? 0 : weights[i - 1]) + (symbols[i].frequencies != null && symbols[i].frequencies.Length > col ? symbols[i].frequencies[col] : 1);
			}
			return weights;
		}
