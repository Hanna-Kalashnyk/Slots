...
namespace CSFramework {
	...
	public class SymbolGen {
		...
		private void MakeNewReels() {
			...
			for (int x = 0; x < reelLength; x++) for (int y = lastIndex; y < symbolsPerReel; y++) reels[x][y] = symbolManager.GetRandomSymbol(x);
		}
		...
		private void Spin() {
			...
			for (int i = 0; i < lines.Length; i++) {
				...
				for (int x = 0; x < reelLength; x++) {
					if (line.paths[x] < 0 || line.paths[x] >= scores[x].Length) continue;
