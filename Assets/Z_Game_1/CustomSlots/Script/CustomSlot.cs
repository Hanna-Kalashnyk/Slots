        ...
        private void Update()
        {
            ...
            switch (state)
            {
                ...
                case State.Result:
                    ...
                    //Minigame Trigger
                    //first searches through the first column for a lion                    
                    for (int i = 0; i < reels[0].GetComponent<Reel>().symbolHolders.Count; i++)
                    {
                        //if lion found, search the next columns for a princess
                        if (reels[0].symbolHolders[i].GetComponent<Image>().sprite.name == reels[0].keySpriteName)
                        {
                            for (int j = 1; j < reels.Length; j++)
                            {
                                for (int k = 0; k < reels[j].symbolHolders.Count; k++)
                                {
                                    //if a princess is found, load the minigame
                                    if (reels[j].symbolHolders[k].GetComponent<Image>().sprite.name == reels[j].keySpriteName)
                                        SceneManager.LoadScene("SG2Mini");
                                    print("blue");
                                }
                            }
                        }
                    ...
                    for (int m = 0; m < reels.Length - 2; m++)
                    {
                        ...
                        {
                            if ((reels[m].symbolHolders[w].GetComponent<Image>().sprite.name == "Sunrise31") && (reels[m + 1].symbolHolders[w].GetComponent<Image>().sprite.name == reels[m].symbolHolders[w].GetComponent<Image>().sprite.name)&&
                    ...
                    {
                        spinN = Elona.Slot.ElosUI.currentSpingNumber;
